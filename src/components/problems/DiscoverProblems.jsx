...
export default function DiscoverProblems() {
  ...
  // Fetch problems using Supabase API with cursor-based pagination
  useEffect(() => {
    const loadProblems = async () => {
      if (!isMounted) return; // Ensure component is mounted before fetching
      ...
      try {
        ...
        params.append('limit', problemsPerPage.toString());
        params.append('sort_by', sortBy === 'timestamp' ? 'created_at' : sortBy);

        if (selectedCategories.length > 0) {
          params.append('category', selectedCategories.join(',')); // Assuming API can handle comma-separated categories
        }
    ...
    loadProblems();
  }, [sortBy, selectedCategories, searchQuery, problemsPerPage, isMounted]); // eslint-disable-line react-hooks/exhaustive-deps
